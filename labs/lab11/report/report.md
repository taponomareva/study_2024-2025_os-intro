...
Открываю редактор командой:
emacs

## Создание файла lab07.sh

Создаётся новый файл комбинацией:
Ctrl-x Ctrl-f

Ввожу имя lab07.sh и нажимаю Enter.
...
Сохраняю изменения с помощью:
Ctrl-x Ctrl-s

# Редактирование текста
## Вырезание строки

Одним нажатием Ctrl-k удаляется текущая строка.

## Вставка строки

Перехожу в конец файла и вставляю строку с помощью Ctrl-y.

## Выделение области

Устанавливаю курсор в нужную точку, нажимаю Ctrl-Space и двигаюсь дальше.
...
Ctrl-y вставляет текст в конец.

## Вырезание области

Выделяется нужная часть и вырезается через Ctrl-w.

## Отмена действия

Если ошибаюсь — отменяю через Ctrl-/.
...
Делится вертикально (Ctrl-x 3), потом каждое окно — по горизонтали (Ctrl-x 2).
...
Нажатием Ctrl-s перебираю найденные совпадения.
...
Запускаю M-s o — Emacs сразу подсвечивает все совпадения. Удобнее, чем классический пошаговый поиск.
